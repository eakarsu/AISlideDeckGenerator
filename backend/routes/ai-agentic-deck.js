// AI Agentic deck generation
// Input topic + data, AI generates complete deck end-to-end
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISlideDeckGenerator. Focus area: Agentic deck generation. ${`Input topic + data, AI generates complete deck end-to-end`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Agentic deck generation.\n${`Input topic + data, AI generates complete deck end-to-end`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'agentic-deck', context, parsed);
    res.json({ feature: 'agentic-deck', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[agentic-deck]', err.message);
